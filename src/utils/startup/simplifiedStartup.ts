...
export function showSimplifiedStartupScreen() {
  try {
    ...
    console.error('Erreur lors de l\'affichage de l\'écran de démarrage simplifié:', error);
    return false;
  }
}
...
export function initializeSafeMode() {
  ...
  if (urlParams.get('mode') === 'safe') {
    ...
    // Désactiver les services locaux si demandé
    if (urlParams.get('noLocalServices') === 'true') {

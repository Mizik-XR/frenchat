  ...
  useEffect(() => {
    const handleAuth = async () => {
      try {
        ...
        // Gestion des erreurs retournées par Microsoft
        if (error) {
          console.error("Erreur OAuth retournée par Microsoft:", error, errorDescription);
          setStatus('error');
          setErrorMessage(`Erreur d'authentification Microsoft: ${error} - ${errorDescription || ''}`);
          toast({
            title: "Échec de l'authentification",
            description: `Microsoft a retourné une erreur: ${error}`,
            variant: "destructive",
          });
          return;
        }
        ...
        if (!code) {
          ...
          toast({
            title: "Échec de l'authentification",
            description: "Code d'autorisation Microsoft manquant",
            variant: "destructive",
          });
          return;
        }
        ...
        const redirectUrl = getRedirectUrl('auth/microsoft/callback');
        const { data, error: exchangeError } = await supabase.functions.invoke('microsoft-oauth', {
          body: { 
            code, 
            redirectUrl,
            state
        ...
        if (exchangeError || !data?.success) {
          throw new Error(exchangeError?.message || data?.error || "Échec de l'échange du code d'autorisation");
        }
        
        const userInfo = data.user_info;
        console.log("Authentification Microsoft réussie pour:", userInfo.email);
        
        setStatus('success');
        toast({
          title: "Authentication réussie",
          description: `Connecté à Microsoft Teams en tant que ${userInfo.email}`,
          variant: "default",
        });
        
        // Redirection après un court délai pour laisser l'utilisateur voir le message de succès
        setTimeout(() => {
          navigate('/config?tab=microsoft-teams', { replace: true });
        }, 1500);
      } catch (error) {
    ...
    <div className="flex h-screen w-full flex-col items-center justify-center p-4">
      <div className="w-full max-w-md rounded-lg border bg-card p-8 shadow-lg">
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold">Authentification Microsoft Teams</h1>
          ...
          {status === 'error' && (
            <div className="mt-6">
              ...
              <Button 
                variant="default" 
                className="w-full" 
                onClick={() => navigate('/config?tab=microsoft-teams')}
              >

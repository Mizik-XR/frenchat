...
interface UncategorizedConversationsSectionProps {
...
export const UncategorizedConversationsSection = ({
  conversations,
  selectedId,
  onSelect,
  ...
  onExportToDoc,
  onDelete
}: UncategorizedConversationsSectionProps) => {
  return (
    <Collapsible defaultOpen className="mb-6">
      <CollapsibleTrigger className="flex items-center gap-2 text-sm font-medium text-gray-500 mb-2 w-full hover:text-gray-700">
        ...
        Non classées ({conversations.length})
      </CollapsibleTrigger>
      <CollapsibleContent>
        <ConversationGroup

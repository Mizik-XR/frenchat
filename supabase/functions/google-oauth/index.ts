...
serve(async (req) => {
  ...
  try {
    ...
    const { code, action, redirectUrl, userId, state } = requestBody;
    ...
    switch (action) {
      ...
      case 'revoke_token':
        ...
        if (tokenError || !tokenData?.refresh_token) {
          console.warn("Aucun token trouvé pour la révocation:", tokenError);
          // On supprime quand même l'entrée dans la BD
          await supabase
            .from('oauth_tokens')
            .delete()
            .eq('user_id', userId)
    ...
    console.error("Erreur lors du traitement de la requête OAuth Google:", error);
    return new Response(
